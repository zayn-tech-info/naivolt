...
export default function GiftCardsPage() {
  ...
  const { data, isLoading, isError } = useQuery<GiftCardTransaction[]>({
    queryKey: ["admin-gift-card-transactions", activeTab],
    queryFn: () =>
      api
        .get("/admin/gift-card-transactions", {
  ...
  return (
    <AdminLayout title="Gift Card Transactions">
      {/* Filter Tabs */}
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        {isLoading ? (
          ...
          <div className="px-6 py-10 text-center text-red-600 text-sm">
            Failed to load gift card transactions.
          </div>
        ) : filtered.length === 0 ? (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-xs text-slate-500 uppercase tracking-wide bg-slate-50 border-b border-slate-100">
                  ...
                  <th className="px-6 py-3 font-medium">Denomination</th>
                  <th className="px-6 py-3 font-medium">Naira Value</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {filtered.map((tx) => (
                  <tr
                    key={tx._id}
                    onClick={() => router.push(`/gift-cards/${tx._id}`)}
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-slate-700">{tx.categoryName ?? "—"}</td>
                    <td className="px-6 py-4 text-slate-700">
                      {tx.currency} {tx.denomination?.toLocaleString()}
                    </td>
